...
const editUser= async (req,res)=>{
    ...
    if(!name&&!email&&!number&&!password) return res.status(400).send({status:false,message:"please pass some data to update" })
    const user={}
    // name
    if(name)
    {
...
// password
    ...
    if(!update) return res.status(400).send({statu:false,message:"no user found"})
  

    return res.status(200).send({statu:true,message:"data updated seccessfully"})
